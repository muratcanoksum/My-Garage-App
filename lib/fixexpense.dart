...
class FixExpenses extends StatefulWidget {
  final int expenseId;
  const FixExpenses({super.key, required this.expenseId});

  @override
  State<StatefulWidget> createState() => _FixExpensesState();
}
...
class _FixExpensesState extends State<FixExpenses> {
  ...
  Future<void> getDataExpense() async {
    await dbHelper.open();
    data2 = await dbHelper.getExpenseData();
    setState(() {
      for (var test in data2) {
        if (test["id"] == widget.expenseId) {
          _nameController.text = test["expenseName"];
          _costController.text = test["expensePrice"];
          selectedCarId = test["carId"].toString();
        }
      }
    });
  }
              ...
              children: [
                ...
                Padding(
                  ...
                  child: Container(
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        dbHelper.deleteExpenseData(widget.expenseId);
                        Navigator.pushReplacement(
                            context,
                            MaterialPageRoute(
                                builder: (context) => const Home()));
